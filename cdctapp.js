...
var fs = require('fs');
const options = {
	key: fs.readFileSync('./keys/2208aielcue/ai_elcue_org_SHA256WITHRSA.key'),
	//key: fs.readFileSync('./keys/elcue/www_elcue_net_SHA256WITHRSA.key'),
	cert:fs.readFileSync('./keys/2208aielcue/ai_elcue_org.crt')
...
vdrg.on('connection',function(socket){
	...
	socket.on('wrssmentortomenteedraw',function(ma){
		...
		if(chk==1){
			socket.broadcast.to(vdrgmanage.userlist[ia].socketid).emit('wrsscopypicsofmentor',{pos:[ma.pos[0],ma.pos[1]],mousestat:ma.mousestat,statoption:ma.statoption});
		}else{
			console.log('user is not exist');
		}*/
