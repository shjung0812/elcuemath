...
io.of('/draw').on('connection', function (socket) {
	...
	socket.on('canvpos', function (pos) {
		...
		// FIX: Use io.of('/draw') to target the correct namespace
		console.log(`[Canvas] Room: ${pos.roomid}, Data:`, pos.data);
		io.of('/draw').in(pos.roomid).emit('canvpos', { data: pos.data });
	});

	// [New] explicitly allow joining rooms (for React migration)
	...
	// --- WebRTC Signaling (New Logic) ---
	socket.on('call_request', function (data) {
		// data: { roomid, type: 'video'|'audio' }
		console.log(`[Signaling] Call Request in ${data.roomid} type: ${data.type}`);
		socket.to(data.roomid).emit('call_request', data);
	});

	socket.on('webrtc_signal', function (data) {
		// data: { roomid, signal: { type, sdp, candidate } }
		// Broadcast to others in room
		socket.to(data.roomid).emit('webrtc_signal', data.signal);
	});

	socket.on('end_call', function (data) {
		console.log(`[Signaling] End Call in ${data.roomid}`);
...
io.on('connection', function (socket) {
	...
	// --- Disconnect Handler (Presence Cleanup) ---
	socket.on('disconnect', function () {
		console.log('[Main/Presence] Socket disconnected:', socket.id);
		// Find and remove user with this socket ID (peerid)
		let removed = false;
		for (var i = 0; i < connectedstatus.length; i++) {
			if (connectedstatus[i][1] === socket.id) {
				const removedUser = connectedstatus[i][0];
				console.log(`[Main/Presence] Removing user: ${removedUser}`);
				connectedstatus.splice(i, 1);
				removed = true;
				break; // Assuming 1:1 user-socket mapping or removing one instance is enough
			}
		}

		if (removed) {
			console.log('[Main/Presence] Broadcasting updated list:', connectedstatus);
			io.emit('mucmresidsent', { csts: connectedstatus });
		}
	});
